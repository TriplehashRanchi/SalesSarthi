  ...
  return (
    ...
    >
      ...
      <div className="flex-1">
        <span className={`block text-sm font-bold ${isActive ? 'text-indigo-900' : 'text-slate-500 group-hover:text-slate-700'}`}>
          {step.title}
        </span>
...
export default function FinancialKundliPage() {
  ...
  const [data, setData] = useState({
    ...
    goals: [{ goal_name: 'Child Higher Education', priority: 'High', current_cost: '', existing_corpus: '', target_year: '' },
          ...
          <div className="px-4 py-4 lg:px-10 lg:py-6 flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-6 lg:gap-12">
               <div>
                  ...
                  <span className={`text-lg lg:text-2xl font-black ${metrics.netWorth < 0 ? 'text-red-500' : 'text-slate-900'}`}>
               ...
               <div>
                  ...
                  <span className="text-lg lg:text-2xl font-black text-slate-900">{metrics.runway} <span className="text-xs font-bold text-slate-400">Months</span></span>
               </div>
            </div>
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="bg-white p-6 lg:p-8 rounded-2xl shadow-sm border border-slate-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <InputGroup label="Financial Doctor" value={data.client.financial_doctor_name} onChange={e => update('client','financial_doctor_name',e.target.value)} placeholder="Advisor Name" />
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="bg-white p-6 lg:p-8 rounded-2xl shadow-sm border border-slate-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <InputGroup label="Monthly Expenses" prefix="₹" value={data.cashflow.monthly_expenses} onChange={e => update('cashflow','monthly_expenses',e.target.value)} />
                    <InputGroup label="Current Monthly Savings" prefix="₹" value={data.cashflow.monthly_savings} onChange={e => update('cashflow','monthly_savings',e.target.value)} />
                    <InputGroup label="Current Emergency Fund" prefix="₹" value={data.cashflow.current_emergency_fund} onChange={e => update('cashflow','current_emergency_fund',e.target.value)} />
                    <InputGroup label="Target Emergency (Months)" value={data.cashflow.desired_emergency_months} onChange={e => update('cashflow','desired_emergency_months',e.target.value)} />
                  </div>
                </div>
                ...
                <div className="bg-white p-6 lg:p-8 rounded-2xl shadow-sm border border-slate-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <InputGroup label="Goal Inflation" suffix="%" value={data.tax_and_risk.inflation_goals_yearly} onChange={e => update('tax_and_risk','inflation_goals_yearly',e.target.value)} />
                    <InputGroup label="Expense Inflation" suffix="%" value={data.tax_and_risk.inflation_expenses_yearly} onChange={e => update('tax_and_risk','inflation_expenses_yearly',e.target.value)} />
                  </div>
                </div>
              </div>
            )}
            ...
            {currentStep === 2 && (
              <div className="bg-white p-6 lg:p-8 rounded-2xl shadow-sm border border-slate-200 animate-in fade-in slide-in-from-bottom-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <InputGroup label="Real Estate (Investment/Self)" prefix="₹" value={data.assets.real_estate} onChange={e => update('assets','real_estate',e.target.value)} className="md:col-span-2" />
                </div>
              </div>
            )}
                ...
                {['home','vehicle','credit_card','personal','other'].map(key => (
                  <div key={key} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                      ...
                      <InputGroup label="Months Left" type="number" value={data.liabilities[key].months_left} onChange={e => updateLoan(key,'months_left',e.target.value)} placeholder="e.g. 24"/>
            ...
            {currentStep === 5 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
                <div className="flex justify-between items-center">
                  ...
                  <button onClick={addGoal} className="flex items-center gap-2 bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100">
                    <Icons.Plus /> Add Goal
                  </button>
                </div>
                  ...
                  {data.goals.map((g, i) => (
                    <div key={i} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm group relative">
                      ...
                      <div className="space-y-4">
                        ...
                        <InputGroup label="Existing Corpus for this Goal" prefix="₹" value={g.existing_corpus} onChange={e => updateGoal(i, 'existing_corpus', e.target.value)} />
                      </div>
                    </div>
                  ))}
                </div>
