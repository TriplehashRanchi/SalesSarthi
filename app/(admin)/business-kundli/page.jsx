...
export default function BusinessKundliWizard() {
  ...
  const [formData, setFormData] = useState({
    ...
    // 4. Client
    client: {
      needs_analysis: 'Sometimes',
      portfolio_checkup: 'Sometimes',
      reviews_per_month: '',
      avg_client_age: '',
      referral_percent: '',
      comm_frequency: 'Quarterly'
    },
    // 5. Operation
    operation: {
      hours_per_week: '',
    ...
    market: {
      ...
      differentiation: [],
      lost_to_competitors: '1-3',
      objection_confidence: 5
    },
    // 7. Tech
    tech: {
      tools: [],
      crm_update: 'Weekly',
      digital_pres: 'Sometimes',
      video_comfort: 'Moderately',
      automation: 'No'
    },
    // 8. Compliance
    compliance: {
  ...
  const renderScreen = () => {
    switch (step) {
      ...
      case 1:
        return (
          <div className="space-y-6 animate-fadeIn">
            <div><h2 className="text-2xl font-bold text-white">Identity</h2><p className="text-slate-300">Tell Us About Yourself.</p></div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><Label>Experience (Yrs)</Label><InputText type="number" value={formData.identity.experience_years} onChange={e => update('identity', 'experience_years', e.target.value)} /></div>
              <div><Label>Work Type</Label><div className="flex gap-2 mt-1">{['Full Time', 'Part Time'].map(t => (<button key={t} onClick={() => update('identity', 'work_type', t)} className={`px-2 py-3 rounded-lg text-xs w-full border ${formData.identity.work_type === t ? 'bg-cyan-500/20 border-cyan-400 text-cyan-200' : 'bg-white/5 border-white/10'}`}>{t}</button>))}</div></div>
            </div>
            <div>
      ...
      case 2:
        return (
          <div className="space-y-6 animate-fadeIn">
            <div><h2 className="text-2xl font-bold text-white">Birthday</h2><p className="text-slate-300">For timing guidance only.</p></div>
            <div className="space-y-4">
               <div><Label required>Date of Birth</Label><InputText type="date" value={formData.birthday.dob} onChange={e => update('birthday', 'dob', e.target.value)} /></div>
               <div><Label required>Time of Birth</Label><InputText type="time" value={formData.birthday.tob} onChange={e => update('birthday', 'tob', e.target.value)} /></div>
               <div><Label required>Place of Birth</Label><InputText value={formData.birthday.pob} onChange={e => update('birthday', 'pob', e.target.value)} /></div>
               <div>
      ...
      case 3:
        return (
          <div className="space-y-6 animate-fadeIn">
            <div><h2 className="text-2xl font-bold text-white">The Numbers</h2><p className="text-slate-300">Business performance metrics.</p></div>
            
            <div className="bg-white/5 p-4 rounded-xl border border-white/10">
              <h3 className="text-indigo-200 text-xs font-bold uppercase tracking-widest mb-4">Last 90 Days</h3>
              <div className="grid grid-cols-2 gap-3">
                 <div><Label>Comm. Earned (₹)</Label><InputText type="number" value={formData.numbers.commission_90} onChange={e => update('numbers', 'commission_90', e.target.value)} /></div>
                 <div><Label>No. of Sales</Label><InputText type="number" value={formData.numbers.sales_count_90} onChange={e => update('numbers', 'sales_count_90', e.target.value)} /></div>
                 <div><Label>Leads Generated</Label><InputText type="number" value={formData.numbers.leads_90} onChange={e => update('numbers', 'leads_90', e.target.value)} /></div>
                 <div><Label>Mtgs Conducted</Label><InputText type="number" value={formData.numbers.meetings_90} onChange={e => update('numbers', 'meetings_90', e.target.value)} /></div>
                 <div><Label>Sales Closed</Label><InputText type="number" value={formData.numbers.sales_closed_90} onChange={e => update('numbers', 'sales_closed_90', e.target.value)} /></div>
                 <div><Label>Avg Ticket Size (₹)</Label><InputText type="number" value={formData.numbers.avg_ticket_90} onChange={e => update('numbers', 'avg_ticket_90', e.target.value)} /></div>
              </div>
            </div>
            ...
            <div className="bg-white/5 p-4 rounded-xl border border-white/10">
              ...
              <div className="grid grid-cols-2 gap-3">
                 <div><Label>Total Income (₹)</Label><InputText type="number" value={formData.numbers.total_income_12m} onChange={e => update('numbers', 'total_income_12m', e.target.value)} /></div>
                 <div><Label>Active Clients</Label><InputText type="number" value={formData.numbers.active_clients} onChange={e => update('numbers', 'active_clients', e.target.value)} /></div>
                 <div><Label>Best Month (₹)</Label><InputText type="number" value={formData.numbers.best_month_income} onChange={e => update('numbers', 'best_month_income', e.target.value)} /></div>
                 <div><Label>Worst Month (₹)</Label><InputText type="number" value={formData.numbers.worst_month_income} onChange={e => update('numbers', 'worst_month_income', e.target.value)} /></div>
              </div>
            </div>
          </div>
        );
      ...
      case 4:
        return (
          <div className="space-y-6 animate-fadeIn">
            <div><h2 className="text-2xl font-bold text-white">Client Health</h2><p className="text-slate-300">Engagement & Behavior.</p></div>
            
            <div className="space-y-4">
              <div>
                <Label>Analyze needs before recommending?</Label>
                <div className="grid grid-cols-4 gap-1 mt-1">{['Never','Rarely','Sometimes','Always'].map(o => <button key={o} onClick={() => update('client', 'needs_analysis', o)} className={`py-2 text-[10px] rounded border ${formData.client.needs_analysis === o ? 'bg-cyan-500/30 border-cyan-400' : 'bg-white/5 border-white/10'}`}>{o}</button>)}</div>
              </div>
              <div>
                <Label>Periodic portfolio check-ups?</Label>
                <div className="grid grid-cols-4 gap-1 mt-1">{['Never','Rarely','Sometimes','Always'].map(o => <button key={o} onClick={() => update('client', 'portfolio_checkup', o)} className={`py-2 text-[10px] rounded border ${formData.client.portfolio_checkup === o ? 'bg-cyan-500/30 border-cyan-400' : 'bg-white/5 border-white/10'}`}>{o}</button>)}</div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                 <div><Label>Review Mtgs/Mo</Label><InputText type="number" value={formData.client.reviews_per_month} onChange={e => update('client', 'reviews_per_month', e.target.value)} /></div>
                 <div><Label>Referral %</Label><InputText type="number" value={formData.client.referral_percent} onChange={e => update('client', 'referral_percent', e.target.value)} /></div>
                 <div><Label>Avg Client Age (Yrs)</Label><InputText type="number" value={formData.client.avg_client_age} onChange={e => update('client', 'avg_client_age', e.target.value)} /></div>
              </div>
              <div>
                <Label>Non-sales Communication Freq</Label>
                <div className="flex gap-2 mt-1 overflow-x-auto">{['Monthly','Quarterly','Yearly','Need Based'].map(o => <SelectChip key={o} label={o} selected={formData.client.comm_frequency === o} onClick={() => update('client', 'comm_frequency', o)} />)}</div>
              </div>
            </div>
          </div>
        );
      ...
      case 5:
        return (
          <div className="space-y-6 animate-fadeIn">
            <div><h2 className="text-2xl font-bold text-white">Operations</h2><p className="text-slate-300">Daily activities & patterns.</p></div>
            
            <div className="grid grid-cols-2 gap-4">
               <div><Label>Hours / Week</Label><InputText type="number" value={formData.operation.hours_per_week} onChange={e => update('operation', 'hours_per_week', e.target.value)} /></div>
               <div><Label>Calls / Day</Label><InputText type="number" value={formData.operation.calls_per_day} onChange={e => update('operation', 'calls_per_day', e.target.value)} /></div>
               <div className="col-span-2"><Label>Meetings / Week</Label><InputText type="number" value={formData.operation.meetings_per_week} onChange={e => update('operation', 'meetings_per_week', e.target.value)} /></div>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {['Referrals', 'Cold/Walk-in', 'Digital', 'Client Reviews', 'Events'].map(s => (
                  <SelectChip key={s} label={s} multi selected={formData.operation.prospecting_sources.includes(s)} onClick={() => toggleArrayItem('operation', 'prospecting_sources', s)} />
                ))}
              </div>
            </div>
          </div>
      ...
      case 6:
        return (
          <div className="space-y-6 animate-fadeIn">
             <div><h2 className="text-2xl font-bold text-white">Market Position</h2><p className="text-slate-300">Competition & Differentiation.</p></div>
             ...
             <div className="my-4">
                ...
                <div className="grid grid-cols-2 gap-2 mt-2">
                  {['Service Quality', 'Product Knowledge', 'Relationships', 'Tech Enabled', '24/7 Availability', 'Holistic Planning'].map(d => (
                    <div key={d} onClick={() => toggleArrayItem('market', 'differentiation', d)} className={`p-3 rounded-lg border text-xs cursor-pointer transition-all ${formData.market.differentiation.includes(d) ? 'bg-fuchsia-500/20 border-fuchsia-400 text-white' : 'border-white/10 text-slate-400'}`}>{d}</div>
                  ))}
                </div>
             </div>
             ...
             <div>
                ...
                <div className="flex gap-2 mt-2">{['None', '1-3', '4-10', '10+'].map(o => <SelectChip key={o} label={o} selected={formData.market.lost_to_competitors === o} onClick={() => update('market', 'lost_to_competitors', o)} />)}</div>
             </div>
          </div>
        );
      ...
      case 7:
        return (
           <div className="space-y-6 animate-fadeIn">
             <div><h2 className="text-2xl font-bold text-white">Technology</h2><p className="text-slate-300">Digital infrastructure.</p></div>
             ...
             <div className="space-y-4 pt-4">
                <div><Label>Update CRM Frequency</Label><div className="flex gap-1 mt-1">{['Daily', 'Weekly', 'Monthly', 'Rarely'].map(o => <button key={o} onClick={() => update('tech', 'crm_update', o)} className={`flex-1 py-2 text-[10px] rounded border ${formData.tech.crm_update === o ? 'bg-indigo-500/40 border-indigo-400' : 'bg-white/5 border-white/10'}`}>{o}</button>)}</div></div>
                <div><Label>Use Digital Presentations?</Label><div className="flex gap-1 mt-1">{['Always', 'Sometimes', 'Rarely', 'Never'].map(o => <button key={o} onClick={() => update('tech', 'digital_pres', o)} className={`flex-1 py-2 text-[10px] rounded border ${formData.tech.digital_pres === o ? 'bg-indigo-500/40 border-indigo-400' : 'bg-white/5 border-white/10'}`}>{o}</button>)}</div></div>
                <div><Label>Video Meeting Comfort</Label><div className="flex gap-1 mt-1 overflow-x-auto">{['Very Comfortable', 'Moderately', 'Learning', 'Avoid'].map(o => <button key={o} onClick={() => update('tech', 'video_comfort', o)} className={`px-2 py-2 text-[10px] whitespace-nowrap rounded border ${formData.tech.video_comfort === o ? 'bg-indigo-500/40 border-indigo-400' : 'bg-white/5 border-white/10'}`}>{o}</button>)}</div></div>
                <div><Label>Automation for follow-ups?</Label><div className="flex gap-4 mt-1"><SelectChip label="Yes" selected={formData.tech.automation === 'Yes'} onClick={() => update('tech', 'automation', 'Yes')} /><SelectChip label="No" selected={formData.tech.automation === 'No'} onClick={() => update('tech', 'automation', 'No')} /></div></div>
             </div>
           </div>
        );
        ...
        return (
          <div className="space-y-6 animate-fadeIn">
             ...
             <div>
               ...
               <div className="flex gap-2 mt-2">{['Monthly', 'Quarterly', 'Yearly', 'Rarely'].map(o => <SelectChip key={o} label={o} selected={formData.compliance.training_freq === o} onClick={() => update('compliance', 'training_freq', o)} />)}</div>
             </div>
          </div>
        );
      ...
      case 9:
        return (
          <div className="space-y-6 animate-fadeIn">
            <div><h2 className="text-2xl font-bold text-white">Skills Assessment</h2><p className="text-slate-300">Rate yourself 1-10.</p></div>
            <div className="space-y-2">
              <RangeSlider label="Product Knowledge" value={formData.skills.product_knowledge} onChange={v => update('skills', 'product_knowledge', v)} subLabels={['Need Work','Avg','Excellent']} />
              <RangeSlider label="Financial Planning" value={formData.skills.fin_planning} onChange={v => update('skills', 'fin_planning', v)} subLabels={['Need Work','Avg','Excellent']} />
              <RangeSlider label="Comparison Ability" value={formData.skills.comparison} onChange={v => update('skills', 'comparison', v)} subLabels={['Need Work','Avg','Excellent']} />
              <RangeSlider label="Communication" value={formData.skills.communication} onChange={v => update('skills', 'communication', v)} subLabels={['Need Work','Avg','Excellent']} />
              ...
              <RangeSlider label="Follow-up Discipline" value={formData.skills.followup} onChange={v => update('skills', 'followup', v)} subLabels={['Need Work','Avg','Excellent']} />
            </div>
      ...
      case 10:
        return (
          <div className="space-y-6 animate-fadeIn">
            <div><h2 className="text-2xl font-bold text-white">Trust & Brand</h2><p className="text-slate-300">Authority indicators.</p></div>
            <div className="space-y-2">
      ...
      case 11:
        return (
          <div className="space-y-6 animate-fadeIn">
            <div><h2 className="text-2xl font-bold text-white">Mindset</h2><p className="text-slate-300">Psychological approach.</p></div>
            <div className="space-y-4">
      ...
      case 12:
        return (
          <div className="flex flex-col items-center justify-center h-[60vh] text-center animate-fadeIn">
            ...
            <p className="text-slate-300">Calculating your Business Grahas</p>
          </div>
        );
    ...
    <div className="min-h-screen relative overflow-hidden bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900 text-slate-50 pb-28 pt-[-40] lg:pb-24">
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-10 pt-6 lg:pt-8">
        ...
        <div className="bg-white/10 border border-white/15 rounded-2xl px-4 py-3 sticky top-4 z-20 flex items-center justify-between gap-3 backdrop-blur-2xl shadow-lg">
          {step > 1 && step < 12 ? (
            <button onClick={() => setStep(s => s - 1)} className="p-2 text-indigo-100 hover:bg-white/10 rounded-full border border-white/10 transition"><IconChevronLeft /></button>
          ) : <div className="w-10" />}
        ...
        <div className="mt-8 mb-20">
          <div className="bg-white/5 border border-white/10 rounded-3xl p-5 sm:p-8 backdrop-blur-xl shadow-2xl min-h-[500px]">
            {renderScreen()}
          </div>
        </div>
