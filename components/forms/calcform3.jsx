...
const calculateInvestmentDiversificationScore = (options, age) => {
    const riskInvestmentKeys = [
        'equityDirectStocks', 'cryptoAssets', 'realEstateProperty', 'ownBusinessStartups',
        'equityMutualFunds', 'derivatives', 'commoditiesForex', 'p2pLending',
        'unlistedSharesPreIPO', 'reitsInvitsThematicETFs',
    ];
    const safeInvestmentKeys = [
        'fixedDepositsRDs', 'ppf', 'epf', 'nps', 'govtBondsAAACorpBonds',
        'postOfficeSchemes', 'sukanyaSamriddhiYojana', 'ulipSavingInsurance',
        'gold', 'debtMutualFundsIndexETFs',
    ];
...
useEffect(() => {
    if (searchParams) { // Ensure searchParams is available
...
const calculateItemScore = (item, prevFormData) => { // prevFormData can be used if a type needs context beyond item
    let percentage = 0;
    let score = 1;
    ...
    setFormData(prev => {
        ...
        let checklistAfterTargetUpdate = prev.checklist.map(item => {
            ...
            switch (tempItem.item) {
                case 'Income Protection': newTarget = annualIncome * 20; break;
                case 'Emergency Fund': newTarget = monthlyExpenses * 3; break;
                case 'Health Insurance': newTarget = annualIncome * 8; break;
                case 'Critical Illness Cover': newTarget = annualIncome * 3; break;
                case 'Disability Insurance': newTarget = annualIncome * 3; break;
                case 'Retirement Goals': newTarget = monthlyExpenses * 300; break;
                case 'Child Education Fund': newTarget = childEducationFundGoal; break;
                case 'Debt Management': newTarget = (annualIncome * 0.4) / 12; break;
                case 'Wealth Planning': newTarget = (annualIncome / 12) * 0.2; break;
                case 'Home Loan or Rent': newTarget = (annualIncome / 12) * 0.3; break;
                case 'Marriage Fund': newTarget = marriageFundGoal; break;
                case "Spouse's Coverage": newTarget = annualIncome * 10; break;
                case 'CIBIL Score': newTarget = 750; break;
                case 'Tax Planning': newTarget = 200000; break;
                case 'Investment Diversification':
                    ...
                    break;
                case 'Budget Planning': case 'Estate Planning': case 'Legacy Fund':
                case 'HUF Account': case 'Family Goals':
        ...
        const finalChecklist = checklistAfterTargetUpdate.map(item => {
            ...
            switch (syncedItem.item) {
                case 'Debt Management': currentValToSyncFromTopLevel = debtManagementEmi; break;
                case 'Wealth Planning': currentValToSyncFromTopLevel = monthlySavings; break;
                case 'Home Loan or Rent': currentValToSyncFromTopLevel = housingCost; break;
                case 'CIBIL Score': currentValToSyncFromTopLevel = cibilScoreCurrent; break;
                case 'Marriage Fund': currentValToSyncFromTopLevel = marriageFundCurrent; break;
                case "Spouse's Coverage": currentValToSyncFromTopLevel = spouseCoverageCurrent; break;
                case 'Tax Planning': currentValToSyncFromTopLevel = taxPlanningCurrent; break;
                case 'Budget Planning': currentValToSyncFromTopLevel = budgetPlanning; break;
                case 'Estate Planning': currentValToSyncFromTopLevel = estatePlanning; break;
                case 'Legacy Fund': currentValToSyncFromTopLevel = legacyFund; break;
                case 'HUF Account': currentValToSyncFromTopLevel = hufAccount; break;
                case 'Family Goals': currentValToSyncFromTopLevel = familyGoals; break;
                default:
...
<div className="mt-4 space-y-2 p-1"> {/* Added space-y-2 and p-1 */}
    <div>
