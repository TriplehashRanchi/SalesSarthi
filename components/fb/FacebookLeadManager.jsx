...
const FacebookLeadManager = () => {
  ...
  useEffect(() => {
    if (authLoading || !firebaseUser || tokenCheckComplete) {
      if (!authLoading && !firebaseUser) setTokenCheckComplete(true); // Mark done if logged out
      return; // Wait for auth, ensure user exists, only run once per login
    }
    ...
    const handleToken = async () => {
        setError(null);
        const tokenFromUrl = searchParams.get('fbToken');
        const expiryFromUrl = searchParams.get('fbExpiry');
        let currentFbToken = null;
        let source = null;

        try {
            const firebaseToken = await firebaseUser.getIdToken(true); // Force refresh if needed

            if (tokenFromUrl) {
                console.log('Found FB token in URL');
                source = 'url';
                currentFbToken = tokenFromUrl;
                try {
                    await axios.post(
                        `${API_URL}/api/admin/updateFbToken`,
                        { fbToken: tokenFromUrl, fbExpiry: expiryFromUrl },
                        { headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${firebaseToken}` } }
                    );
                    localStorage.setItem('fbAccessToken', tokenFromUrl);
                    if (isMounted) router.replace('/facebook-leads', undefined, { shallow: true });
                } catch (dbError) {
                    console.error('Error updating FB token in DB:', dbError);
                    if (isMounted) setError('Failed to save Facebook connection. Please try logging in again.');
                    currentFbToken = null; // Don't use if save failed
                    source = null;
                }
            } else {
                console.log('No FB token in URL, checking DB...');
                try {
                    // Try DB first
                    ...
                    if (response.data && response.data.fbToken) {
                        ...
                        source = 'db';
                        localStorage.setItem('fbAccessToken', currentFbToken); // Sync local
                    } else {
                    ...
                    // Fallback to LocalStorage even on DB error
                    const storedToken = localStorage.getItem('fbAccessToken');
                    if (storedToken) {
                        console.log('Found FB token in LocalStorage after DB error');
                        currentFbToken = storedToken;
                        source = 'local';
                    } else {
                        console.log('No FB token found anywhere after DB error.');
                    }
                }
            }
            ...
            if (isMounted) {
                ...
                setTokenCheckComplete(true); // Mark check as complete
                // *** CRITICAL CHANGE: Fetch SAVED selections now ***
            ...
            if (isMounted) {
                ...
                setAuthLoading(false);
                setTokenCheckComplete(true); // Mark as done even on error
            }
        }
    };
