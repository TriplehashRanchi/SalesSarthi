            ...
            <ul className="mb-5 flex border-b">
                {tabs.map((t) => (
                    <li key={t.id} className="inline-block">
                        <button onClick={() => setActiveTab(t.id)} className={`flex gap-2 border-b-2 p-4 ${activeTab === t.id ? 'border-primary text-primary' : 'border-transparent'}`}>
                            {t.icon}
                            {t.label}
                        </button>
                    </li>
                    ...
                    <div className="flex flex-col sm:flex-row">
                        <div className="mb-4 sm:mr-4">
                            <label htmlFor="avatarInput">
                                <img src={avatarPreview || '/assets/images/profile-34.jpeg'} alt="avatar" className="h-20 w-20 rounded-full object-cover cursor-pointer" />
                            </label>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 flex-1">
                            ...
                            <div>
                                ...
                                <select id="country" className="form-select" value={profile.country} onChange={(e) => handleProfileChange('country', e.target.value)}>
                            ...
                            <div>
                                ...
                                <input id="address" className="form-input" placeholder="Address" value={profile.address} onChange={(e) => handleProfileChange('address', e.target.value)} />
                            </div>
                            <div className="mt-6 border-t pt-5 sm:col-span-2">
                                {!showPasswordForm ? (
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="flex gap-3">
                                            <button type="button" className="btn btn-primary" disabled={savingPassword} onClick={changePassword}>
                            ...
                            <div className="sm:col-span-2">
                                <button type="button" className="btn btn-primary mt-2" onClick={() => saveProfileInfo()}>
                                    Save Profile
                                </button>
                            </div>
                        </div>
                ...
                <div className="bg-white p-6 rounded border">
                    ...
                    <div className="mb-4">
                        ...
                        <div className="h-24 w-24 border flex items-center justify-center">
                            {logoPreview ? <img src={logoPreview} alt="logo" className="h-full object-contain" /> : <span>No logo</span>}
                        </div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {[
                            ...
                            <div key={f.id}>
                                <label htmlFor={f.id}>{f.label}</label>
                                <input id={f.id} className="form-input bg-gray-100 cursor-not-allowed" value={company[f.id] || 'N/A'} disabled />
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded border max-w-md">
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            {subscription.expires_at && <p className="text-sm">Expires on {new Date(subscription.expires_at).toLocaleDateString()}</p>}
                        </div>
                    </div>
                </div>
            )}
