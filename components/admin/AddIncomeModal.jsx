...
export default function AddIncomeModal({ isOpen, onClose, onSave }) {
  ...
  const handleSubmit = async () => {
    if (!form.month || !form.income) {
      alert("Please select a month and enter income amount.");
      return;
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="px-6 py-4 bg-gray-50 border-t flex justify-end gap-3">
          <button onClick={onClose} className="px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-200 rounded-lg transition">Cancel</button>
          <button 
