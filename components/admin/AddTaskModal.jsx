...
export default function AddTaskModal({ isOpen, onClose, onSave, agents }) {
    ...
    const handleSubmit = async () => {
        if (!formData.title || !formData.agent_id || !formData.due_date) {
            toast.error('Please fill in Title, Agent, and Due Date');
            return;
        }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
                ...
                <div className="px-6 py-4 bg-gray-50 border-t flex justify-end gap-3">
                    ...
                    <button onClick={handleSubmit} className="px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg">
                        Create Task
                    </button>
                </div>
            </div>
